...
- **The webview is torn down 120 s after the last request** (`idleTeardownDelay`), below the 5-minute refresh interval, so between refreshes no WebKit process is alive at all. The store is `.nonPersistent()`, so the next refresh re-solves the challenge — that costs a couple of seconds in the background.
